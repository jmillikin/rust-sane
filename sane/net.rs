...
impl ControlOptionRequest {
	...
	pub fn action(&self) -> crate::Action {
		self.inner.action
	}
...
impl io::Encode for ControlOptionRequest {
	fn encode<W: io::Write>(
		...
		ProcedureNumber::CONTROL_OPTION.encode(w)?;
		self.handle().encode(w)?;
		Word::new(self.option()).encode(w)?;
		self.action().encode(w)?;
		if self.action() != crate::Action::SET_AUTO {
			self.value().encode(w)?;
		}
		Ok(())
	}
...
impl io::Decode for ControlOptionRequestBuf {
	fn decode<R: io::Read>(
		...
		let mut buf = ControlOptionRequestBuf::new();
		buf.set_handle(Handle::decode(r)?);
		buf.set_option(Word::decode(r)?.as_u32());
		buf.set_action(crate::Action::decode(r)?);
		if buf.action() != crate::Action::SET_AUTO {
...
impl io::Decode for ControlOptionReplyBuf {
	fn decode<R: io::Read>(
		...
		let mut buf = ControlOptionReplyBuf::new();
		buf.set_status(crate::Status::decode(r)?);
...
impl OptionValue<'_> {
	pub const BUTTON: OptionValue<'static> = OptionValue {
		value_type: crate::ValueType::BUTTON,
		bytes: &[],
	};
}
...
impl<'a> OptionValue<'a> {
	...
	pub fn to_fixed(self) -> Result<crate::Fixed, OptionValueError> {
		Ok(crate::Fixed::from_word(Word::new(self.to_u32()?)))
	}

	#[cfg(any(doc, feature = "alloc"))]
	pub fn to_fixed_list(self) -> Result<Vec<crate::Fixed>, OptionValueError> {
		let mut bytes = self.bytes;
...
impl OptionValueBuf {
	...
	pub fn from_i32(value: i32) -> OptionValueBuf {
		OptionValueBuf {
			value_type: crate::ValueType::INT,
			bytes: Vec::from(value.to_be_bytes()),
		}
	}
	...
	pub fn from_fixed(value: crate::Fixed) -> OptionValueBuf {
		OptionValueBuf {
			value_type: crate::ValueType::FIXED,
			bytes: Vec::from(value.as_word().as_u32().to_be_bytes()),
		}
	}

	pub fn from_fixed_list(values: &[crate::Fixed]) -> OptionValueBuf {
		let mut bytes = Vec::with_capacity(4 * values.len());
		for value in values {
